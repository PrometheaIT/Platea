...
var environment = {
  production: false,
  supabaseUrl: "https://qhjsweueviqnrlpqozee.supabase.co",
  supabaseKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFoanN3ZXVldmlxbnJscHFvemVlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjMwMTk0MTksImV4cCI6MjA3ODU5NTQxOX0.uG_C-MMiRvTNRLS6j6mGagVgqWo_fWhkuU0GrM4GAQA",
  vapidPublicKey: "BKa5TIFEQDNHXC-mWhIz_8_v7Us1pSMsCOqEKSeh1ej-eqmrlxDMJZW14MXOlanb-h9UF_nZ20Q1SdMhzOyworw",
  // Timbrature - progetto separato
  timbratureSupabaseUrl: "https://qgtquamtmzqvytxwlwst.supabase.co",
  timbratureSupabaseKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFndHF1YW10bXpxdnl0eHdsd3N0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjE2NTU0NzMsImV4cCI6MjA3NzIzMTQ3M30.xR4HjqI1lYoFLgbZl1zJ4eLRXjM_uraIdK2l_6t2G4A"
};
...
// src/app/services/Base/supabase.service.ts
var SupabaseService = class _SupabaseService {
    ...
    return __async(this, null, function* () {
      try {
        const { data: authData, error: authError } = yield this.supabase.auth.signUp({
          ...
          options: {
            data: {
              first_name: credentials.user_data.first_name,
              last_name: credentials.user_data.last_name,
              phone: credentials.user_data.phone,
              role: credentials.user_data.role
            }
          }
        });
        if (authError)
        ...
        yield this.ensureUserInUsersTable(supabaseUser.id, {
          email: credentials.email,
          first_name: credentials.user_data.first_name,
          last_name: credentials.user_data.last_name,
          phone: credentials.user_data.phone || null,
          role: credentials.user_data.role
        });
        const user = this.convertSupabaseUserToAppUser(supabaseUser, credentials.user_data);
        let restaurant = null;
        if (credentials.user_data.role === "restaurant_owner" && credentials.restaurant_data) {
          restaurant = yield this.createRestaurant(supabaseUser.id, credentials);
        }
        yield this.handleStaffInvitations(supabaseUser.id, credentials.email);
        return { user, restaurant, supplier: null, error: void 0 };
      } catch (error) {
        console.error("\u274C SignUp error:", error);
        return { user: null, restaurant: null, supplier: null, error: error.message };
      }
    });
  }
  signIn(email, password) {
    return __async(this, null, function* () {
      return yield this.supabase.auth.signInWithPassword({ email, password });
    });
  }
  signOut() {
    return __async(this, null, function* () {
      return yield this.supabase.auth.signOut();
    });
  }
  getCurrentUser() {
  ...
  /**
   * Ottiene il profilo utente dalla tabella 'users'. Se non esiste, lo crea automaticamente.
   */
  getUserProfileWithFallback(userId, authUser) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabase.from("users").select("*").eq("id", userId).single();
        if (!error && data) {
          return data;
        }
        if (error?.code === "PGRST116") {
          ...
          if (!authUser) {
            const { data: { user } } = yield this.getCurrentUser();
            authUser = user || void 0;
          }
          if (!authUser)
            return null;
          const userData = {
          ...
          const created = yield this.ensureUserInUsersTable(userId, userData);
          if (created) {
            const { data: newData } = yield this.supabase.from("users").select("*").eq("id", userId).single();
            return newData;
          }
        }
        return null;
      } catch (error) {
        console.error("Error in getUserProfileWithFallback:", error);
        return null;
      }
    });
  }
  /**
   * Crea un record nella tabella 'users' se non esiste già.
   */
  ensureUserInUsersTable(userId, userData) {
    return __async(this, null, function* () {
      try {
        const { data: existing, error: checkError } = yield this.supabase.from("users").select("id").eq("id", userId).maybeSingle();
        if (checkError) {
        ...
        if (!existing) {
          ...
          const { error: insertError } = yield this.supabase.from("users").insert(payload);
          if (insertError) {
  ...
  // ==================== STAFF INVITATIONS ====================
  handleStaffInvitations(userId, email) {
    return __async(this, null, function* () {
      try {
        const { data: pendingInvites, error } = yield this.supabase.from("restaurant_staff").select("*").eq("email", email.toLowerCase()).eq("invitation_status", "pending").is("user_id", null);
        if (error || !pendingInvites?.length)
          return;
        for (const invite of pendingInvites) {
          yield this.supabase.from("restaurant_staff").update({ user_id: userId, invitation_status: "accepted" }).eq("id", invite.id);
        }
      } catch (error) {
        console.error("Error in handleStaffInvitations:", error);
      }
    });
  }
  // ==================== RESTAURANT METHODS ====================
  getCurrentUserRestaurant() {
    return __async(this, null, function* () {
      try {
        ...
        if (!user)
          return null;
        const { data, error } = yield this.supabase.from("restaurants").select("*").eq("owner_id", user.id).maybeSingle();
        return error ? null : data;
      } catch (error) {
        console.error("Error in getCurrentUserRestaurant:", error);
        return null;
      }
    });
  }
  getCurrentUserSupplier() {
    return __async(this, null, function* () {
      try {
        ...
        if (!user)
          return null;
        const { data, error } = yield this.supabase.from("suppliers").select("*").eq("user_id", user.id).maybeSingle();
        return error ? null : data;
      } catch (error) {
        console.error("Error in getCurrentUserSupplier:", error);
        return null;
      }
    });
  }
  ...
  getRestaurantPhotos(restaurantId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("restaurant_photos").select("*").eq("restaurant_id", restaurantId).order("display_order", { ascending: true });
      if (error)
        throw error;
      return (data || []).map((photo) => __spreadProps(__spreadValues({}, photo), {
        url: photo.file_path ? this.supabase.storage.from("restaurant-images").getPublicUrl(photo.file_path).data.publicUrl : void 0
      }));
    });
  }
  uploadRestaurantPhoto(restaurantId, file, displayOrder) {
    return __async(this, null, function* () {
      try {
        if (file.size > 10 * 1024 * 1024)
          throw new Error("File troppo grande (max 10MB)");
        if (!file.type.startsWith("image/"))
          throw new Error("Solo file immagine sono permessi");
        const timestamp = Date.now();
        const sanitizedName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
        const filePath = `restaurants/${restaurantId}/gallery/${timestamp}_${sanitizedName}`;
        const { error: uploadError } = yield this.supabase.storage.from("restaurant-images").upload(filePath, file, { cacheControl: "3600", upsert: false });
        if (uploadError) {
          if (uploadError.message.includes("already exists")) {
            yield this.supabase.storage.from("restaurant-images").remove([filePath]);
            const { error: retryError } = yield this.supabase.storage.from("restaurant-images").upload(filePath, file, { upsert: true });
            if (retryError)
              throw retryError;
          } else {
            throw uploadError;
        ...
        const publicUrl = this.supabase.storage.from("restaurant-images").getPublicUrl(filePath).data.publicUrl;
  ...
  saveRestaurantPhotoRecord(photoData) {
    return __async(this, null, function* () {
      try {
        yield this.supabase.from("restaurant_photos").delete().eq("restaurant_id", photoData.restaurant_id).eq("display_order", photoData.display_order);
        const { data, error } = yield this.supabase.from("restaurant_photos").insert([__spreadProps(__spreadValues({}, photoData), {
          created_at: (/* @__PURE__ */ new Date()).toISOString(),
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
        })]).select().single();
        return error ? null : data;
  ...
  deleteRestaurantPhoto(photoId, filePath) {
    return __async(this, null, function* () {
      try {
        yield this.supabase.from("restaurant_photos").delete().eq("id", photoId);
        yield this.supabase.storage.from("restaurant-images").remove([filePath]).catch(() => {
        });
        return true;
      } catch (error) {
  ...
  updatePhotoOrder(photoId, displayOrder) {
    return __async(this, null, function* () {
      try {
        yield this.supabase.from("restaurant_photos").update({ display_order: displayOrder, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", photoId);
        return true;
      } catch (error) {
  ...
  // ==================== PRIVATE HELPERS ====================
  createRestaurant(userId, credentials) {
    return __async(this, null, function* () {
      const payload = {
        owner_id: userId,
        name: credentials.restaurant_data.name,
        address: credentials.restaurant_data.address || "",
        phone: credentials.restaurant_data.phone || "",
        email: credentials.restaurant_data.email || credentials.email,
        is_active: true,
        created_at: (/* @__PURE__ */ new Date()).toISOString(),
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      };
      const { data, error } = yield this.supabase.from("restaurants").insert(payload).select().single();
      if (error)
        throw error;
      return data;
    });
  }
  convertSupabaseUserToAppUser(supabaseUser, userData) {
    ...
    return {
      ...
      first_name: userData.first_name || "",
      last_name: userData.last_name || "",
      phone: userData.phone || void 0,
      role: userData.role || "customer",
      avatar_url: supabaseUser.user_metadata?.["avatar_url"] || void 0,
      email_verified: !!supabaseUser.email_confirmed_at,
      created_at: supabaseUser.created_at || now,
      updated_at: supabaseUser.updated_at || now,
      user_metadata: supabaseUser.user_metadata || {},
      is_active: true,
      address: supabaseUser.user_metadata?.["address"] || void 0,
      preferences: supabaseUser.user_metadata?.["preferences"] || void 0
    };
  }
  sendPromotionPush(promotionId, restaurantId) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabase.functions.invoke("send-promotion-push", {
          body: { promotion_id: promotionId, restaurant_id: restaurantId }
        });
        if (error) {
          console.error("\u274C Error calling send-promotion-push:", error);
          return { success: false, error };
        }
        return { success: true, data };
      } catch (err) {
        console.error("\u274C Exception calling send-promotion-push:", err);
...
// src/app/shared/guards/permission.service.ts
var PermissionService = class _PermissionService {
  supabaseService = inject(SupabaseService);
  userPermissionsSubject = new BehaviorSubject([]);
  userPermissions$ = this.userPermissionsSubject.asObservable();
  // CACHE
  permissionsCache = /* @__PURE__ */ new Map();
  // Lista completa di tutti i permessi disponibili
  allPermissions = [
    { key: "view_dashboard", label: "Visualizza Dashboard", description: "Accesso alla dashboard principale" },
    { key: "manage_menu", label: "Gestione Men\xF9", description: "Aggiungere, modificare e eliminare piatti e categorie" },
    { key: "manage_orders", label: "Gestione Ordini", description: "Visualizzare e gestire gli ordini dei clienti" },
    { key: "manage_reservations", label: "Gestione Prenotazioni", description: "Gestire le prenotazioni dei tavoli" },
    { key: "view_reports", label: "Visualizza Report", description: "Accesso ai report finanziari e statistiche" },
    { key: "view_kitchen_display", label: "Pannello Cucina", description: "Visualizzare gli ordini in cucina" },
    { key: "manage_inventory", label: "Gestione Inventario", description: "Gestire ingredienti e scorte" },
    { key: "view_expenses", label: "Visualizza Spese", description: "Visualizzare e gestire le spese" },
    { key: "view_table_map", label: "Mappa Tavoli", description: "Visualizzare la mappa dei tavoli" },
    { key: "take_orders", label: "Prendere Ordini", description: "Registrare nuovi ordini" },
    { key: "manage_restaurant_settings", label: "Impostazioni Ristorante", description: "Modificare le impostazioni del ristorante" },
    { key: "manage_staff", label: "Gestione Staff", description: "Aggiungere e gestire il personale" },
    { key: "view_ingredients", label: "Visualizza Ingredienti", description: "Accesso alla gestione ingredienti e ricette" },
    { key: "view_menu_readonly", label: "Visualizza Men\xF9", description: "Visualizzazione men\xF9 in sola lettura" },
    { key: "manage_dish_photos", label: "Gestione Foto Piatti", description: "Aggiungere, modificare e rimuovere foto dei piatti" },
    { key: "manage_marketing", label: "Gestione Marketing", description: "Accesso e gestione della sezione marketing e promozioni" },
    { key: "view_profile", label: "Visualizza Profilo", description: "Visualizzazione dati del profilo senza modifiche" },
    { key: "view_calendar_readonly", label: "Visualizza Calendario", description: "Visualizzazione calendario prenotazioni in sola lettura" },
    { key: "manage_timbrature", label: "Gestione Timbrature", description: "Gestire timbrature, dipendenti, turni e report presenze" },
    { key: "view_timbrature", label: "Visualizza Timbrature", description: "Registrare la propria timbratura" }
  ];
  /**
   * Carica i permessi per un utente specifico
   */
  loadUserPermissions(userId, restaurantId, isOwner, staffRole, isSupplier) {
    return __async(this, null, function* () {
      console.log("\u{1F504} PermissionService - Caricamento permessi:", {
        userId,
        restaurantId,
        isOwner,
        staffRole,
      ...
      if (isSupplier) {
        const supplierPermissions = [
          ...
          "view_supplier_restaurants",
          "manage_supplier_orders",
          "manage_supplier_profile"
        ];
        this.userPermissionsSubject.next(supplierPermissions);
        console.log("\u2705 Permessi fornitore caricati");
        return;
      }
      const cacheKey = isOwner ? `owner_${restaurantId}` : `${restaurantId}_${staffRole}`;
      if (this.permissionsCache.has(cacheKey)) {
        const cached = this.permissionsCache.get(cacheKey);
        this.userPermissionsSubject.next(cached);
        console.log("\u2705 Permessi dalla cache:", cached);
        return;
      }
      if (isOwner) {
        const ownerPermissions = this.allPermissions.filter((p) => !p.key.includes("supplier")).map((p) => p.key);
        this.permissionsCache.set(cacheKey, ownerPermissions);
        this.userPermissionsSubject.next(ownerPermissions);
        console.log("\u2705 Permessi proprietario caricati (e messi in cache):", ownerPermissions);
        return;
      }
      if (staffRole && restaurantId) {
        const permissions = yield this.getPermissionsForRole(restaurantId, staffRole);
        this.permissionsCache.set(cacheKey, permissions);
        this.userPermissionsSubject.next(permissions);
        console.log("\u2705 Permessi staff caricati (e messi in cache):", permissions);
        return;
      }
      this.userPermissionsSubject.next([]);
      console.log("\u26A0\uFE0F Nessun permesso caricato");
    });
  }
  invalidateCache(restaurantId, role, isOwner) {
    const key = isOwner ? `owner_${restaurantId}` : `${restaurantId}_${role}`;
   ...
   */
  getPermissionsForRole(restaurantId, role) {
    return __async(this, null, function* () {
      try {
        console.log("\u{1F50D} Loading permissions for role:", { restaurantId, role });
        const { data: staffData, error: staffError } = yield this.supabaseService.getSupabaseClient().from("restaurant_staff").select("permissions").eq("restaurant_id", restaurantId).eq("role", role).eq("invitation_status", "accepted").maybeSingle();
        if (!staffError && staffData?.permissions && staffData.permissions.length > 0) {
          console.log("\u2705 Permissions loaded from staff table:", staffData.permissions);
          return staffData.permissions;
        }
        const { data: rolePermsData, error: rolePermsError } = yield this.supabaseService.getSupabaseClient().from("restaurant_role_permissions").select("permissions").eq("restaurant_id", restaurantId).eq("role", role).maybeSingle();
        if (!rolePermsError && rolePermsData?.permissions && rolePermsData.permissions.length > 0) {
          console.log("\u2705 Permissions loaded from role_permissions table:", rolePermsData.permissions);
          return rolePermsData.permissions;
        }
        console.log("\u{1F4ED} No custom permissions found, using defaults for role:", role);
  ...
  hasAllPermissions(permissionKeys) {
    const currentPermissions = this.userPermissionsSubject.getValue();
    return permissionKeys.every((key) => currentPermissions.includes(key));
  }
  /**
   * Ottieni i permessi attuali dell'utente
   */
  getCurrentPermissions() {
    return this.userPermissionsSubject.getValue();
  }
  /**
   * Imposta i permessi manualmente (per testing)
   */
  setPermissions(permissions) {
    this.userPermissionsSubject.next(permissions);
  }
  /**
  * Verifica se l'utente può accedere a una route basata sui permessi
  */
  ...
  /**
   * Ricarica i permessi forzando un refresh dal database
   */
  reloadPermissions(userId, restaurantId, isOwner, staffRole, isSupplier) {
    return __async(this, null, function* () {
      yield this.loadUserPermissions(userId, restaurantId, isOwner, staffRole, isSupplier);
    });
  }
  /**
   * Verifica se l'utente è proprietario del ristorante
   */
  isOwner() {
    return false;
  }
...
var AuthService = class _AuthService {
  ...
  // --- Subjects privati (stato) ---
  userSubject = new BehaviorSubject(null);
  staffRecordsSubject = new BehaviorSubject([]);
  restaurantsSubject = new BehaviorSubject([]);
  supplierSubject = new BehaviorSubject(null);
  activeRestaurantIdSubject = new BehaviorSubject(null);
  activeStaffRecordSubject = new BehaviorSubject(null);
  contextSubject = new BehaviorSubject("customer");
  authInitialized = false;
  isLoadingUserData = false;
  // --- Observables pubblici (per i componenti) ---
  currentUser$ = this.userSubject.asObservable();
  currentStaff$ = this.activeStaffRecordSubject.asObservable();
  // staff attivo
  currentRestaurant$ = this.activeRestaurantIdSubject.pipe(switchMap((id) => id ? this.getRestaurantById$(id) : of(null)));
  currentContext$ = this.contextSubject.asObservable();
  allStaffRecords$ = this.staffRecordsSubject.asObservable();
  // tutti i record staff
  availableRestaurants$ = this.restaurantsSubject.asObservable();
  // tutti i ristoranti accessibili
  supplier$ = this.supplierSubject.asObservable();
  activeRestaurantId$ = this.activeRestaurantIdSubject.asObservable();
  // SIDEBAR
  sidebarCollapsedSubject = new BehaviorSubject(true);
  sidebarCollapsed$ = this.sidebarCollapsedSubject.asObservable();
  // Manteniamo i getter per compatibilità con i componenti esistenti
  ...
  // per retrocompatibilità (se usato)
  constructor() {
    this.initializeAuth();
  }
  // ------------------------------------------------------------------
  ...
  initializeAuth() {
    return __async(this, null, function* () {
      try {
        const { data: { session } } = yield this.supabaseService.getSession();
        if (session?.user) {
          const user = yield this.supabaseService.getUserProfileWithFallback(session.user.id, session.user);
        ...
        console.warn("\u26A0\uFE0F Auth init had issues:", e);
        this.userSubject.next(null);
        this.setContext("customer");
      } finally {
  ...
  // ------------------------------------------------------------------
  // Caricamento completo dati utente
  // ------------------------------------------------------------------
  loadUserData(userId) {
    return __async(this, null, function* () {
      ...
      try {
        console.log("\u{1F50D} AUTH LOAD USER DATA START", { userId });
        const [ownedRestaurants, staffRecords, supplier] = yield Promise.all([
          this.findOwnedRestaurants(userId),
          this.findAllStaffRecords(userId, this.userSubject.value?.email),
          this.findSupplierMembership(userId)
        ]);
        console.log("\u{1F50D} Owned restaurants:", ownedRestaurants);
        console.log("\u{1F50D} Staff records:", staffRecords);
        this.staffRecordsSubject.next(staffRecords);
        console.log("\u{1F50D} Supplier:", supplier);
        this.supplierSubject.next(supplier);
        const allRestaurantIds = [
          ...ownedRestaurants.map((r) => r.id),
          ...staffRecords.map((sr) => sr.restaurant_id)
        ];
        const uniqueIds = [...new Set(allRestaurantIds)];
        let allRestaurants = [...ownedRestaurants];
        if (uniqueIds.length > 0) {
          const { data, error } = yield this.supabaseService.getSupabaseClient().from("restaurants").select("*").in("id", uniqueIds);
          if (!error && data) {
            const fetchedRestaurants = data;
            allRestaurants = [...ownedRestaurants, ...fetchedRestaurants.filter((r) => !ownedRestaurants.some((o) => o.id === r.id))];
          }
        }
        console.log("\u{1F50D} All restaurants:", allRestaurants);
        this.restaurantsSubject.next(allRestaurants);
        let activeId = this.getStoredActiveRestaurantId();
        if (!activeId || !allRestaurants.some((r) => r.id === activeId)) {
          activeId = allRestaurants[0]?.id || null;
        }
        yield this.setActiveRestaurant(activeId);
        let defaultContext = "customer";
        if (allRestaurants.length > 0)
          defaultContext = "staff";
        else if (supplier)
        ...
        const contextToApply = savedContext && availableContexts.includes(savedContext) ? savedContext : defaultContext;
        this.setContext(contextToApply);
  ...
  getUserProfile(userId) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabaseService.getSupabaseClient().from("users").select("*").eq("id", userId).single();
        if (error)
          throw error;
        return data;
      } catch (error) {
  ...
  findOwnedRestaurants(userId) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabaseService.getSupabaseClient().from("restaurants").select("*").eq("owner_id", userId);
        if (error)
          throw error;
        return data || [];
      } catch (error) {
        console.error("Error finding owned restaurants:", error);
        return [];
      }
    });
  }
  findAllStaffRecords(userId, email) {
    return __async(this, null, function* () {
      if (!email)
        return [];
      try {
        const { data, error } = yield this.supabaseService.getSupabaseClient().from("restaurant_staff").select("*").or(`user_id.eq.${userId},email.eq.${email.toLowerCase()}`).in("invitation_status", ["pending", "accepted"]);
        if (error)
          throw error;
        const pendingIds = (data || []).filter((r) => r.invitation_status === "pending" && !r.user_id).map((r) => r.id);
        if (pendingIds.length > 0) {
          const { error: updateError } = yield this.supabaseService.getSupabaseClient().from("restaurant_staff").update({ user_id: userId, invitation_status: "accepted" }).in("id", pendingIds);
          if (!updateError) {
            (data || []).forEach((r) => {
              if (pendingIds.includes(r.id)) {
                r.user_id = userId;
                r.invitation_status = "accepted";
              }
            });
          }
        }
        ...
        console.error("Error finding staff records:", error);
        return [];
      }
    });
  }
  getRestaurantById(restaurantId) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabaseService.getSupabaseClient().from("restaurants").select("*").eq("id", restaurantId).single();
        if (error)
          throw error;
        return data;
      } catch (error) {
  ...
  getRestaurantById$(restaurantId) {
    return new Observable((observer) => {
      this.getRestaurantById(restaurantId).then((restaurant) => {
        observer.next(restaurant);
        observer.complete();
      }).catch((err) => observer.error(err));
    });
  }
  findSupplierMembership(userId) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabaseService.getSupabaseClient().from("suppliers").select("*").eq("user_id", userId).maybeSingle();
        if (error)
          throw error;
        return data;
      } catch (error) {
  ...
  setActiveRestaurant(restaurantId) {
    return __async(this, null, function* () {
      ...
      if (restaurantId) {
        const staffRecord = this.staffRecordsSubject.value.find((sr) => sr.restaurant_id === restaurantId) || null;
        this.activeStaffRecordSubject.next(staffRecord);
        localStorage.setItem("activeRestaurantId", restaurantId);
      } else {
        this.activeStaffRecordSubject.next(null);
        localStorage.removeItem("activeRestaurantId");
      }
    });
  }
  // ------------------------------------------------------------------
  // API pubbliche (invariate o leggermente modificate)
  // ------------------------------------------------------------------
  login(email, password, rememberMe = false) {
    return __async(this, null, function* () {
      try {
        const { data, error } = yield this.supabaseService.signIn(email, password);
        if (error)
          return { data: null, error };
        if (data.user) {
          const user = yield this.getUserProfile(data.user.id);
          this.userSubject.next(user);
          ...
          if (isDeactivated) {
            const hasRestaurants = this.restaurantsSubject.value.length > 0;
            if (hasRestaurants) {
              this.setContext("staff");
              this.router.navigate(["/restaurant/dashboard"]);
            } else {
              this.router.navigate(["/customer/profile-user"]);
            }
            return { data, error: null };
          }
          const context = this.getCurrentContext();
          const route = context === "staff" ? "/restaurant/dashboard" : context === "supplier" ? "/supplier/dashboard" : "/customer/home";
  ...
  register(credentials) {
    return __async(this, null, function* () {
      try {
        const result = yield this.supabaseService.signUp(credentials);
        if (result.user && !result.error) {
          this.userSubject.next(result.user);
          yield this.loadUserData(result.user.id);
          yield this.loadPermissions();
          const context = this.getCurrentContext();
          const route = context === "staff" ? "/restaurant/dashboard" : "/customer";
          this.router.navigate([route]);
        }
        return result;
      } catch (error) {
        console.error("\u274C Register exception:", error);
        return { user: null, error, restaurant: null, supplier: null };
      }
    });
  }
  logout() {
    return __async(this, null, function* () {
      try {
        yield this.supabaseService.signOut();
      } catch (e) {
        console.warn("Logout error", e);
      }
      this.userSubject.next(null);
      this.staffRecordsSubject.next([]);
  ...
  refreshUserData() {
    return __async(this, null, function* () {
      ...
      if (!user)
        return;
      yield this.loadUserData(user.id);
    });
  }
  // ------------------------------------------------------------------
  // Gestione contesto e ristoranti
  // ------------------------------------------------------------------
  getCurrentContext() {
  ...
  getAvailableContexts() {
    ...
    if (this.restaurantsSubject.value.length > 0)
      contexts.push("staff");
    if (this.supplierSubject.value)
  ...
  switchRestaurant(restaurantId) {
    return __async(this, null, function* () {
      const exists = this.restaurantsSubject.value.some((r) => r.id === restaurantId);
      if (!exists)
        return false;
      yield this.setActiveRestaurant(restaurantId);
      yield this.loadPermissions();
      return true;
    });
  }
  // ------------------------------------------------------------------
  // Metodi usati da BaseService e altri
  // ------------------------------------------------------------------
  getCurrentRestaurantId() {
    return this.activeRestaurantIdSubject.value;
  }
  getCurrentStaffRestaurantId() {
    return this.activeRestaurantIdSubject.value;
  }
  getCurrentStaff() {
    return this.activeStaffRecordSubject.value;
  }
  getStaffRole() {
    return this.activeStaffRecordSubject.value?.role || null;
  }
  isOwner() {
    const user = this.userSubject.value;
    const activeId = this.activeRestaurantIdSubject.value;
    if (!user || !activeId)
      return false;
    const restaurant = this.restaurantsSubject.value.find((r) => r.id === activeId);
  ...
  isSupplierUser() {
    return this.contextSubject.value === "supplier";
  }
  isCustomerUser() {
    return this.contextSubject.value === "customer";
  }
  isRestaurantUser() {
    return this.contextSubject.value === "staff";
  }
  canBeStaff() {
    return this.restaurantsSubject.value.length > 0;
  }
  canBeSupplier() {
  ...
  getUserRole() {
    if (!this.userSubject.value)
      return null;
    switch (this.contextSubject.value) {
      case "staff":
        return this.isOwner() ? "restaurant_owner" : this.getStaffRole();
      case "supplier":
        return "supplier";
      default:
        return "customer";
  ...
  // ------------------------------------------------------------------
  // Gestione permessi (delega a PermissionService)
  // ------------------------------------------------------------------
  loadPermissions() {
    return __async(this, null, function* () {
      const user = this.userSubject.value;
      const restaurantId = this.activeRestaurantIdSubject.value;
      const isOwner = this.isOwner();
      const staffRole = this.getStaffRole();
      const isSupplier = this.isSupplierUser();
      yield this.permissionService.loadUserPermissions(user?.id || null, restaurantId, isOwner, staffRole, isSupplier);
    });
  }
  hasPermission(permissionKey) {
  ...
  hasAnyPermission(permissionKeys) {
    return this.permissionService.hasAnyPermission(permissionKeys);
  }
  hasAnyPermissions() {
    return this.permissionService.getCurrentPermissions().length > 0;
  }
  getCurrentPermissions() {
    return this.permissionService.getCurrentPermissions();
  }
  refreshPermissions() {
    return __async(this, null, function* () {
      yield this.loadPermissions();
  ...
  // ------------------------------------------------------------------
  isAuthenticated() {
    return this.authInitialized && !!this.userSubject.value;
  }
  waitForAuthInit() {
    return __async(this, null, function* () {
  ...
  printStaffDebugInfo() {
    console.log("\u{1F50D} AuthService Debug:", {
      user: this.userSubject.value?.id,
      activeRestaurantId: this.activeRestaurantIdSubject.value,
      activeStaff: this.activeStaffRecordSubject.value,
      allStaffRecords: this.staffRecordsSubject.value,
      restaurants: this.restaurantsSubject.value,
      supplier: this.supplierSubject.value,
  ...
  getAllStaffRecords() {
    return this.staffRecordsSubject.value;
  }
  /**
   * Verifica se l'utente corrente ha ruolo manager
   */
  isManager() {
    return this.getStaffRole() === "manager";
  }
  /**
   * Verifica se l'utente corrente ha ruolo chef
   */
  isChef() {
    return this.getStaffRole() === "chef";
  }
  /**
   * Verifica se l'utente corrente ha ruolo waiter
   */
  isWaiter() {
    return this.getStaffRole() === "waiter";
  }
  /**
   * Verifica se l'utente corrente ha ruolo kitchen_staff
   */
  isKitchenStaff() {
    return this.getStaffRole() === "kitchen_staff";
  }
  // SIDEBAR
  ...
  // ------------------------------------------------------------------
  /**
   * Disattiva il profilo utente (soft delete).
   * Setta is_active=false e deactivated_at=now().
   */
  deactivateUserAccount() {
    return __async(this, null, function* () {
      ...
      const { error } = yield this.supabaseService.getSupabaseClient().from("users").update({
        is_active: false,
      ...
      this.userSubject.next(__spreadProps(__spreadValues({}, user), { is_active: false }));
      return { success: true };
    });
  }
  /**
   * Riattiva il profilo utente.
   */
  reactivateUserAccount() {
    return __async(this, null, function* () {
      ...
      if (error)
        return { success: false, error: error.message };
      this.userSubject.next(__spreadProps(__spreadValues({}, user), { is_active: true }));
      return { success: true };
    });
  }
  /**
   * Elimina definitivamente il profilo utente.
   * Prima elimina da users (cascade sui dati), poi dall'auth di Supabase.
   */
  deleteUserAccount() {
    return __async(this, null, function* () {
      const user = this.userSubject.value;
      if (!user)
        return { success: false, error: "Utente non autenticato" };
      const { error: dbError } = yield this.supabaseService.getSupabaseClient().from("users").delete().eq("id", user.id);
      if (dbError)
  ...
  /**
   * Disattiva un ristorante (solo owner).
   */
  deactivateRestaurant(restaurantId) {
    return __async(this, null, function* () {
      if (!this.isOwner())
        return { success: false, error: "Non autorizzato" };
      const { error } = yield this.supabaseService.getSupabaseClient().from("restaurants").update({
        is_active: false,
        deactivated_at: (/* @__PURE__ */ new Date()).toISOString(),
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("id", restaurantId).eq("owner_id", this.userSubject.value.id);
      if (error)
        return { success: false, error: error.message };
      yield this.refreshUserData();
      return { success: true };
  ...
  /**
   * Elimina definitivamente un ristorante (solo owner).
   */
  deleteRestaurant(restaurantId) {
    return __async(this, null, function* () {
      if (!this.isOwner())
        return { success: false, error: "Non autorizzato" };
      const { error } = yield this.supabaseService.getSupabaseClient().from("restaurants").delete().eq("id", restaurantId).eq("owner_id", this.userSubject.value.id);
      if (error)
        return { success: false, error: error.message };
      yield this.refreshUserData();
      if (this.restaurantsSubject.value.length === 0) {
  ...
  /**
   * Verifica se l'utente attivo ha il profilo personale disattivato.
   */
  isUserAccountDeactivated() {
    return this.userSubject.value?.is_active === false;
  }
...
//# sourceMappingURL=chunk-JWP3QZ4U.js.map
